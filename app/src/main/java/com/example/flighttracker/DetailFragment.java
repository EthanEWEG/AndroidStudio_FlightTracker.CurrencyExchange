...
import com.example.flighttracker.databinding.FragmentDetailBinding;
...
//data key for indicating if this flight is saved
    ...
    private void deleteFlight() {
        try {
            Collection collection = FlightApplication.getCollection();
            Query query = QueryBuilder.select(SelectResult.all())
                    .from(DataSource.collection(collection))
                    .where(Expression.property(FlightApplication.KEY_ID)
            ...
            for (Result result : resultSet) {
                ...
                String docID = dictionary.getString(FlightApplication.KEY_DOC_ID);
                assert docID != null;
    ...
    private void saveFlight() {
        if (isSaved()) {
            try {
                ...
                mutableDocument.setString(FlightApplication.KEY_DATA, flightJson);
                mutableDocument.setString(FlightApplication.KEY_ID, getCurrentFlightID());
                collection.save(mutableDocument);
                String docID = mutableDocument.getId();
                //save doc id
                mutableDocument.setString(FlightApplication.KEY_DOC_ID, docID);
                collection.save(mutableDocument);
